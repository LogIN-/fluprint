 ...
 * @Last Modified time: 2019-05-21 14:35:23
 */
...
class PHPFunctions {
	...
	public static function sanitizeMedicalHistoryData($string, $column) {
		...
		$mapping = array(
			...
			"iiv injection" => 2,
			"doesn't know" => 3,
			"laiv_std_intranasal" => 4,
			"laiv_std_ intranasal" => 4,
			"laiv4 intranasal" => 4,
			"im" => 2,
			"doesn`t remember" => 3,
			"n/a" => 3,
		...
		foreach ($mapping as $mappingKey => $mappingValue) {
			if ($mappingKey === strtolower($string)) {
				$data = $mappingValue;
				break;
			}
		}
		...
		if (is_numeric($string)) {
			$data = filter_var($string, FILTER_SANITIZE_NUMBER_INT);
		}

		if ($data === "") {
			$data = "Unknown - sanitizeMedicalHistoryData - " . $string . " - " . $column;
			var_dump($data);
			exit;
		}
	...
	public static function sanitizeVaccinationData($string) {
		...
		$mapping = array(
			...
			"Fluzone 0.25 mL IIV4 SD syringe" => 5, // "Pediatric Fluzone",
			...
			"Fluzone 0.25 mL single-dose syringe" => 5, // "Pediatric Fluzone",
	...
	public static function sanitizeAssay($string) {
		...
		$mapping = array("CMV EBV" => 1,
			...
			"Other Luminex" => 16,
			"Phenotyping-1" => 17,
			"pCyTOF (whole blood) v2 pheno" => 18,
			"pCyTOF (whole blood) v2 phospho" => 19,
			"Viral Neutralization (Mt. Sinai)" => 20);
			...
			foreach ($database["experimental_data"][$donor_visits_id] as $donor_visit_data) {
				...
				if (isset($donor_visits[$donor_visit_id][$donor_visit_data["name"]])) {
					...
					if (isset($main_values["post"]) && isset($main_values["pre"])) {
						if ($main_values["post"] > 0 && $main_values["pre"] > 0) {
					...
					// exit;
					if ($delta_single !== "NA" && is_infinite($delta_single)) {
						$delta_single = "NA";
					}
