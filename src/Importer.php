...
class Importer {
	...
	public function clearDatabase() {
		$tables = ['donors', 'donor_visits', 'experimental_data', 'medical_history'];
	...
	public function processVisitInformation() {
		...
		foreach ($this->donorData as $studyID => $studyDonors) {
			...
			foreach ($studyDonors as $donorId => $donorVisits) {
				...
				if (!isset($this->donor->insertedDataTracker["donors.medical_history"])) {
					Logger::write()->addError("INFO: Missing information for medical_history. Not inserting medical_history for donor.");
				}
			}
		}

		Logger::write()->addError("INFO: Removing donors that doesn't have visits data.");
		$this->deleteMissingDonors();
	}

	/**
	 * [deleteMissingDonors description]
	 * @return [type] [description]
	 */
	public function deleteMissingDonors() {
		$sql = "DELETE FROM donors WHERE  donors.id NOT IN(SELECT DISTINCT dv.donor_id FROM donor_visits dv)";
		...
		Logger::write()->addError("INFO: Removed donor_ids: " . $stmt->rowCount());
	}
